...
@app.route('/')
def home():
    return {"status": "Backend funcionando con BigQuery"}
...
def query_data():
    try:
        ...
        # Consulta inteligente a BigQuery
        ...
        Datos reales de BigQuery tabla Adereso_WebhookTests: {data_summary}
        
        Responde en español basándote en los datos reales mostrados.
        """
        
        response = model.generate_content(prompt)
        
        # Generar gráfico con datos reales
        chart_data = None
        if len(results) > 0:
